...
import com.sena.springecommerce.service.IUsruarioService;
...
public class APIProductoController {
	...
	@Autowired
	private IUsruarioService usruarioService;

	// Endpoint GET para obtener todos los productos
	@GetMapping("/list")
	public List<Producto> getAllPrdocuts() {
		return productoService.findAll();
	}

	// Endpoit Get para obtener un prducto por ID
	@GetMapping("/Product/{id}")
	public ResponseEntity<Producto> getProductById(@PathVariable Integer id) {
		Optional<Producto> produto = productoService.get(id);
	...
	public ResponseEntity<Producto> createProduct(@RequestBody Producto producto) {
		Usuario u = usruarioService.findById(1).get();
	...
	public ResponseEntity<Producto> updateProduct(@PathVariable Integer id, @RequestBody Producto productDetails) {
		...
		// mantener imagen existente amenos que se envie una nueva
